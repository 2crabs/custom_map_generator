...
fn main() {
    ...
    for i in mat_index.clone(){
        let color = reduce_material(i.0, i.1.1, "color.png");
...
fn reduce_material(material_color: [u8; 4], max_colors: i32, file: &str) -> ImageBuffer<Rgb<u8>, Vec<u8>>{
    let mut all_colors = get_num_colors(material_color);
...
fn get_common_colors() -> Vec<color_reduction::image::Rgb<u8>>{
    ...
    let mut materials: HashMap<[u8;4], i32> = HashMap::new();
    materials.insert([255,255,255,255], 8);
    materials.insert([0,255,0,255], 16);
    materials.insert([255,0,0,255], 16);
    materials.insert([128,128,128,255], 16);
    materials.insert([255,255,0,255], 16);
    materials.insert([255,0,255,255], 16);
    materials.insert([0,255,255,255], 16);
    materials.insert([0,0,255,255], 16);
    materials.insert([128,0,0,255], 16);
    materials.insert([0,128,0,255], 16);
    materials.insert([0,0,128,255], 16);
    materials.insert([128,128,0,255], 8);
    materials.insert([0,128,128,255], 8);
    for material in materials{
        let mut all_colors = get_num_colors(material.0);
        all_colors.truncate(material.1 as usize);
        colors.append(&mut convert_to_rgb(all_colors))
    }
